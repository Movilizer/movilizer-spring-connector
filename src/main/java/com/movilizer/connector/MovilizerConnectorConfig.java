...
public class MovilizerConnectorConfig
{
    ...
    @Value("${movilizer.charset}") String charset) throws MalformedURLException
  {
    ...
    return Movilizer.buildConf().setEndpoint(endpoint).setOutputEncoding(
      Charset.forName(charset)).getService();
  }

  /**
   * This method shows how alternative endpoints can be configured. Infact the demo, prod and d
   * environment are supported by the standard implementation. This code however need improvement
   * since the user needs to be able to set this from his configuration file if needed.
   *
   * @param movilizerEnv
   * @param charset
   * @return
    ...
    throws MalformedURLException
  {
    ...
    switch (movilizerEnv)
    {
      ...
      case "prod":
        mds = Movilizer.buildConf().setEndpoint(EndPoint.PROD).setOutputEncoding(
          Charset.forName(charset)).getService();
        break;
      case "d":
        mds = Movilizer.buildConf().setEndpoint(
          "http://d.movilizer.com/MovilizerDistributionService/WebService/",
          "http://d.movilizer.com/mds/document").setOutputEncoding(
            Charset.forName(charset)).getService();
        break;
      case "epcis":
        mds = Movilizer.buildConf().setEndpoint(
          "https://epcis-test.movilizer.com/MovilizerDistributionService/WebService/",
